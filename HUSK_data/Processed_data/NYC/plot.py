...
df_area = pd.read_csv('NYC_area.csv')
...
df_poi = pd.read_csv('NYC_poi.csv')
...
# colors = ['red', 'green', 'blue', 'purple', 'orange', 'yellow', 'pink', 'gray', 'brown', 'black', 'white']
#
...
#         ).add_to(m)
############################
# 5. 绘制功能区的POI点（仅限 Lincoln 区域内，但保持功能区颜色区分）
############################

# 提取 Lincoln 区域的 geometry（支持多个 polygon）
lincoln_geom = gdf_area[gdf_area['zone'].str.contains("Lincoln")]['geometry']
lincoln_union = lincoln_geom.unary_union  # 合并为一个 geometry

# 筛选落在 Lincoln 区域内的 POI（from 已聚合功能区结果）
gdf_lincoln_poi = gdf_merged[gdf_merged['geometry'].within(lincoln_union)]

# 设定颜色（如功能区数多于颜色数，自动循环）
colors = ['blue']
# colors = ['red', 'green', 'blue', 'purple', 'orange', 'yellow', 'pink', 'gray', 'brown', 'black', 'white']

# 使用功能区ID区分颜色并绘图
unique_zone_ids = gdf_lincoln_poi['functional_zone_id'].unique()
zone_color_map = {fz_id: colors[i % len(colors)] for i, fz_id in enumerate(sorted(unique_zone_ids))}

for _, poi in gdf_lincoln_poi.iterrows():
    lat, lng = poi['lat'], poi['lng']
    fz_id = poi['functional_zone_id']
    folium.CircleMarker(
        location=[lat, lng],
        radius=5,
        color=zone_color_map[fz_id],
        fill=True,
        fill_opacity=0.7
    ).add_to(m)
