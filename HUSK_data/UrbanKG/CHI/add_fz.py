...
def main():
    # ========== 0. 路径设定 ==========
    base_dir = ""
    processed_dir = "../../Processed_data/CHI"

    # 原始文件
    kg_file = os.path.join(base_dir, "UrbanKG_CHI.txt")
    entity2id_file = os.path.join(base_dir, "entity2id_CHI.txt")

    #### 测试不同参数文件 ####
    cluster_result_file = os.path.join(processed_dir, "cluster_result_alpha0.50_beta0.50.csv")

    # 输出文件
    kg_withfz_file = os.path.join(base_dir, "UrbanKG_CHI_withFZ.txt")
    entity2id_withfz_file = os.path.join(base_dir, "entity2id_CHI.txt")
