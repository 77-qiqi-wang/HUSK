...
class KGModel(nn.Module, ABC):
    ...
    def get_ranking(self, queries, filters, batch_size=1000, DAPTA_PATH=None):
        ...
        """
        ranks = torch.ones(len(queries))#.cuda()
        entitydict = {"POI":[], "PC":[], "Area": [], "Road":[], "Junction":[], "FZ":[], "RC":[], "JC":[], "Borough":[]}
        entityids = open(DAPTA_PATH+"/entity2id_NYC.txt", "r", encoding="utf-8").readlines()
        for e in entityids:
            entitydict[e.split("/")[0]].extend([int(e.strip().split(" ")[1])])
        del entityids
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                ...
                for i, query in enumerate(these_queries):
                    ...
                    filter_out.add(queries[b_begin + i, 2].item())
                    # pdb.set_trace()
                    ks = [k for k in entitydict.keys() if queries[b_begin + i, 2].item() in entitydict[k]][0]

                    wrong_types = []
                    for k in entitydict.keys():
                        if k != ks:
                            wrong_types.extend(entitydict[k])

                    # pdb.set_trace()
                    filter_out.update(wrong_types)
                    # pdb.set_trace()
                    filter_tensor = torch.LongTensor(list(filter_out))
                    scores[i, filter_tensor] = -1e6
                    del wrong_types, filter_out, filter_tensor
                    # import gc
                    # gc.collect()
                # print("Get Rankings")
    ...
    def compute_metrics(self, examples, filters, batch_size=500, DAPTA_PATH=None):
        ...
        for m in ["rhs"]: #,"lhs"
            ...
            if m == "lhs":
                ...
                q[:, 2] = tmp
                q[:, 1] += self.sizes[1] // 2
            ranks = self.get_ranking(q, filters[m], batch_size=batch_size, DAPTA_PATH=DAPTA_PATH)
